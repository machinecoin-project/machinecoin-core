...
unsigned int GetNextWorkRequired_V2(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Only change once per interval
    if ((pindexLast->nHeight+1) % Params().IntervalV2() != 0)
    {
        if (Params().AllowMinDifficultyBlocks())
        {
            // Special difficulty rule for testnet:
            ...
            else
            {
                // Return the last non-special-min-difficulty-rules-block
                const CBlockIndex* pindex = pindexLast;
                while (pindex->pprev && pindex->nHeight % Params().IntervalV2() != 0 && pindex->nBits == nProofOfWorkLimit)
                    pindex = pindex->pprev;
                return pindex->nBits;
            }
        }
    ...
    int64_t nActualTimespan = pindexLast->GetBlockTime() - pindexFirst->GetBlockTime();
    LogPrintf("  nActualTimespan = %d  before bounds\n", nActualTimespan);
    if (nActualTimespan < (Params().TargetTimespanV2() - (Params().TargetTimespanV2()/4)))
        nActualTimespan = (Params().TargetTimespanV2() - (Params().TargetTimespanV2()/4));
    if (nActualTimespan > (Params().TargetTimespanV2() + (Params().TargetTimespanV2()/2)))
        nActualTimespan = (Params().TargetTimespanV2() + (Params().TargetTimespanV2()/2));
    ...
    LogPrintf("Params().TargetTimespanV2() = %d    nActualTimespan = %d\n", Params().TargetTimespanV2(), nActualTimespan);
    LogPrintf("Before: %08x  %s\n", pindexLast->nBits, bnOld.ToString());
    LogPrintf("After:  %08x  %s\n", bnNew.GetCompact(), bnNew.ToString());
