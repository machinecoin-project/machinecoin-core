...
CMutableTransaction::CMutableTransaction(const CTransaction& tx) : vin(tx.vin), vout(tx.vout), nType(tx.nType), nVersion(tx.nVersion), nLockTime(tx.nLockTime), vExtraPayload(tx.vExtraPayload) {}
