...
uint256 hashGenesisBlock("0x6a1f879bcea5471cbfdee1fd0cb2ddcc4fed569a500e352d41de967703e83172");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Machinecoin: starting difficulty is 1 / 2^12
...
/** Fees smaller than this (in microcents) are considered zero fee (for transaction creation) */
int64 CTransaction::nMinTxFee = 100000;
/** Fees smaller than this (in microcents) are considered zero fee (for relaying) */
...
const string strMessageMagic = "Machinecoin Signed Message:\n";
...
{
    // Machinecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Machinecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

// Initial starting specs for retarget
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Machinecoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Machinecoin: 2.5 minutes
static const int64 nInterval = nTargetTimespan / nTargetSpacing;

// Specs after block NDIFF_START_DIGISHIELD
static const int64 nTargetTimespanV2 = 150;
static const int64 nTargetSpacingV2 = 150;
...
unsigned int GetNextWorkRequired_V2(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // thanks to RealSolid & WDC for the basis of this code, modified by Mammix2 for Granitecoin
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if (fTestNet) {
        if (pindexLast->nHeight+1 >= NDIFF_START_DIGISHIELD_TESTNET) { DiffMode = 2; } // change the retarget in testnet
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Machinecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Machinecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6a1f879bcea5471cbfdee1fd0cb2ddcc4fed569a500e352d41de967703e83172");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1389040865;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3716037;
        ...
        assert(block.hashMerkleRoot == uint256("0x36a9e41063f3e71466299d0ed9e8193c1c802a88b286016fa4a4d0c3bc384a5c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Machinecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MachinecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MachinecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MachinecoinMiner(CWallet *pwallet)
{
    printf("MachinecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("machinecoin-miner");
    ...
    try { loop {
        ...
        printf("Running MachinecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MachinecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MachinecoinMiner, pwallet));
}
