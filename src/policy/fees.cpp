                                         ...
                                         unsigned int nBlockHeight, EstimationResult *result) const
{
    ...
    LogPrint(MCLog::ESTIMATEFEE, "FeeEst: %d %s%.0f%% decay %.5f: feerate: %g from (%g - %g) %.2f%% %.1f/(%.1f %d mem %.1f out) Fail: (%g - %g) %.2f%% %.1f/(%.1f %d mem %.1f out)\n",
...
void TxConfirmStats::Read(CAutoFile& filein, int nFileVersion, size_t numBuckets)
{
    ...
    LogPrint(MCLog::ESTIMATEFEE, "Reading estimates: %u buckets counting confirms up to %u blocks\n",
...
void TxConfirmStats::removeTx(unsigned int entryHeight, unsigned int nBestSeenHeight, unsigned int bucketindex, bool inBlock)
{
    ...
    if (nBestSeenHeight == 0)  // the BlockPolicyEstimator hasn't seen any blocks yet
        blocksAgo = 0;
    if (blocksAgo < 0) {
        LogPrint(MCLog::ESTIMATEFEE, "Blockpolicy error, blocks ago is negative for mempool tx\n");
    ...
    if (blocksAgo >= (int)unconfTxs.size()) {
        if (oldUnconfTxs[bucketindex] > 0) {
            ...
            LogPrint(MCLog::ESTIMATEFEE, "Blockpolicy error, mempool tx removed from >25 blocks,bucketIndex=%u already\n",
                     bucketindex);
        }
    }
    else {
        ...
        if (unconfTxs[blockIndex][bucketindex] > 0) {
            ...
            LogPrint(MCLog::ESTIMATEFEE, "Blockpolicy error, mempool tx removed from blockIndex=%u,bucketIndex=%u already\n",
...
void CBlockPolicyEstimator::processTransaction(const CTxMemPoolEntry& entry, bool validFeeEstimate)
{
    ...
    if (mapMemPoolTxs.count(hash)) {
        LogPrint(MCLog::ESTIMATEFEE, "Blockpolicy error mempool tx %s already being tracked\n",
                 hash.ToString().c_str());
        return;
    }
    ...
    // Feerates are stored and reported as MAC-per-kb:
...
bool CBlockPolicyEstimator::processBlockTx(unsigned int nBlockHeight, const CTxMemPoolEntry* entry)
{
    ...
    if (blocksToConfirm <= 0) {
        ...
        LogPrint(MCLog::ESTIMATEFEE, "Blockpolicy error Transaction had negative blocksToConfirm\n");
        return false;
    }
                                         ...
                                         std::vector<const CTxMemPoolEntry*>& entries)
{
    ...
    if (firstRecordedHeight == 0 && countedTxs > 0) {
        ...
        LogPrint(MCLog::ESTIMATEFEE, "Blockpolicy first recorded height %u\n", firstRecordedHeight);
    }


    LogPrint(MCLog::ESTIMATEFEE, "Blockpolicy estimates updated by %u of %u block txs, since last block %u of %u tracked, mempool map size %u, max target %u from %s\n",
...
void CBlockPolicyEstimator::FlushUnconfirmed(CTxMemPool& pool) {
    ...
    LogPrint(MCLog::ESTIMATEFEE, "Recorded %u unconfirmed txs from mempool in %gs\n",txids.size(), (endclear - startclear)*0.000001);
}
