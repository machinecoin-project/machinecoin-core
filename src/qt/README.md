This directory contains the MachinecoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
### machinecoingui.(h/cpp)

Represents the main window of the Machinecoin UI.
...
ClientModel is used by the main application `machinecoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/machinecoin/machinecoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a machinecoin:... link from a browser).
...
* UI elements like MachinecoinAmountField, which inherit from QWidget.
* `machinecoinstrings.cpp`: automatically generated
* `machinecoinunits.(h/cpp)`: MAC / mMAC / etc handling
* `callback.h`
...
* don't change `local/machinecoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "machinecoin-qt" under "Run", which is where you can also add command line arguments)
