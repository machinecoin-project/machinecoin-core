...
void AdvertiseLocal(CNode *pnode)
{
    if (fListen && pnode->fSuccessfullyConnected)
    {
        ...
        {
            LogPrint(MCLog::NET, "AdvertiseLocal: advertising address %s\n", addrLocal.ToString());
...
static CAddress GetBindAddress(SOCKET sock)
{
    ...
    if (sock != INVALID_SOCKET) {
        if (!getsockname(sock, (struct sockaddr*)&sockaddr_bind, &sockaddr_bind_len)) {
            ...
            LogPrint(MCLog::NET, "Warning: getsockname failed\n");
        }
    }
    return addr_bind;
}
...
CNode* CConnman::ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure, bool manual_connection)
{
    ...
    /// debug print
    LogPrint(MCLog::NET, "trying connection %s lastseen=%.1fhrs\n",
    ...
    if (pszDest) {
        ...
        if (Lookup(pszDest, resolved,  default_port, fNameLookup && !HaveNameProxy(), 256) && !resolved.empty()) {
            ...
            if (!addrConnect.IsValid()) {
                LogPrint(MCLog::NET, "Resolver returned invalid address %s for %s\n", addrConnect.ToString(), pszDest);
                return nullptr;
            }
...
void CConnman::DumpBanlist()
{
    ...
    LogPrint(MCLog::NET, "Flushed %d banned node ips/subnets to banlist.dat  %dms\n",
...
void CNode::CloseSocketDisconnect()
{
    ...
    if (hSocket != INVALID_SOCKET)
    {
        LogPrint(MCLog::NET, "disconnecting peer=%d\n", id);
        CloseSocket(hSocket);
    }
}
    ...
    bool notifyUI = false;
    {
        ...
        while(it != setBanned.end())
        {
            ...
            if(now > banEntry.nBanUntil)
            {
                ...
                setBannedIsDirty = true;
                notifyUI = true;
                LogPrint(MCLog::NET, "%s: Removed banned node ip/subnet from banlist.dat: %s\n", __func__, subNet.ToString());
            }
            else
                ++it;
        }
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Machinecoin users should be well used to small numbers with many decimal places by now :)
...
bool CNode::ReceiveMsgBytes(const char *pch, unsigned int nBytes, bool& complete)
{
    ...
    while (nBytes > 0) {
        ...
        if (msg.in_data && msg.hdr.nMessageSize > MAX_PROTOCOL_MESSAGE_LENGTH) {
            LogPrint(MCLog::NET, "Oversized message from peer=%i, disconnecting\n", GetId());
            return false;
        }
...
void CConnman::AcceptConnection(const ListenSocket& hListenSocket) {
    ...
    {
        LogPrint(MCLog::NET, "connection from %s dropped (banned)\n", addr.ToString());
        CloseSocket(hSocket);
        return;
    }
    ...
    if (nInbound >= nMaxInbound)
    {
        if (!AttemptToEvictConnection()) {
            ...
            LogPrint(MCLog::NET, "failed to find an eviction candidate - connection dropped (full)\n");
            CloseSocket(hSocket);
            return;
        }
    }
    ...
    LogPrint(MCLog::NET, "connection from %s accepted\n", addr.ToString());
            ...
            if (!fNetworkActive) {
                ...
                for (CNode* pnode : vNodes) {
                    if (!pnode->fDisconnect) {
                        LogPrint(MCLog::NET, "Network not active, dropping peer=%d\n", pnode->GetId());
                        pnode->fDisconnect = true;
                    }
                }
            }
        ...
        for (CNode* pnode : vNodesCopy)
        {
            ...
            if (recvSet || errorSet)
            {
                ...
                else if (nBytes == 0)
                {
                    ...
                    if (!pnode->fDisconnect) {
                        LogPrint(MCLog::NET, "socket closed\n");
                    }
                    pnode->CloseSocketDisconnect();
                }
                else if (nBytes < 0)
            ...
            if (nTime - pnode->nTimeConnected > 60)
            {
                ...
                {
                    LogPrint(MCLog::NET, "socket no message in first 60 seconds, %d %d from %d\n", pnode->nLastRecv != 0, pnode->nLastSend != 0, pnode->GetId());
                    pnode->fDisconnect = true;
                }
...
static void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Machinecoin " + FormatFullVersion();
...
void CConnman::DumpAddresses()
{
    ...
    LogPrint(MCLog::NET, "Flushed %d addresses to peers.dat  %dms\n",
...
void CConnman::SetTryNewOutboundPeer(bool flag)
{
    ...
    LogPrint(MCLog::NET, "net: setting try another outbound peer=%s\n", flag ? "true" : "false");
}
        ...
        if (addrConnect.IsValid()) {
            ...
            if (fFeeler) {
                ...
                if (!interruptNet.sleep_for(std::chrono::milliseconds(randsleep)))
                    return;
                LogPrint(MCLog::NET, "Making feeler connection to %s\n", addrConnect.ToString());
            }
...
{
    LogPrint(MCLog::NET, "SetNetworkActive: %s\n", active);
...
bool CConnman::Start(CScheduler& scheduler, const Options& connOptions)
{
    ...
    if (bandb.Read(banmap)) {
        ...
        LogPrint(MCLog::NET, "Loaded %d banned node ips/subnets from banlist.dat  %dms\n",
            banmap.size(), GetTimeMillis() - nStart);
    } else {
    ...
    nSendVersion(0)
{
    ...
    if (fLogIPs) {
        ...
        LogPrint(MCLog::NET, "Added connection peer=%d\n", id);
    }
}
...
void CNode::AskFor(const CInv& inv)
{
    ...
    if (it != mapAlreadyAskedFor.end())
        ...
        nRequestTime = 0;
    LogPrint(MCLog::NET, "askfor %s  %d (%s) peer=%d\n", inv.ToString(), nRequestTime, DateTimeStrFormat("%H:%M:%S", nRequestTime/1000000), id);
...
void CConnman::PushMessage(CNode* pnode, CSerializedNetMsg&& msg)
{
    ...
    LogPrint(MCLog::NET, "sending %s (%d bytes) peer=%d\n",  SanitizeString(msg.command.c_str()), nMessageSize, pnode->GetId());
