...
Machinecoin Core has supported automatically pruning the blockchain since 0.11. Pruning
...
When Machinecoin Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `machinecoin-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`machinecoin-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/machinecoin-cli -named help command="help"
    src/machinecoin-cli -named getblockhash height=0
    src/machinecoin-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/machinecoin-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
- #8722 `bb843ad` machinecoin-cli: More detailed error reporting (laanwj)
...
- #8830 `ef0801b` test: Add option to run machinecoin-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to machinecoin-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to machinecoin-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and machinecoin-util-test.py (jnewbery)
...
- #9691 `fc67cd2` Init ECC context for `test_machinecoin_fuzzy` (gmaxwell)
...
- #9865 `289204f` Change machinecoin address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` machinecoind: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow machinecoin-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` machinecoin-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to machinecoin-tx (jnewbery)
