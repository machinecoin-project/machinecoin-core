...
*Setup instructions for a Gitian build of Machinecoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Machinecoin
...
- [Building Machinecoin Core](#building-machinecoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-machinecoin-host/README.md).
...
Clone the git repositories for machinecoin and Gitian.
...
in the machinecoin repository under 'Fetch and create inputs' to install sources which require
...
To build Machinecoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the machinecoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/machinecoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/machinecoin-project/machinecoin
...
For example:
```bash
URL=https://github.com/thrasher-/machinecoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit machinecoin=${COMMIT} --url machinecoin=${URL} ../machinecoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit machinecoin=${COMMIT} --url machinecoin=${URL} ../machinecoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit machinecoin=${COMMIT} --url machinecoin=${URL} ../machinecoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the machinecoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../machinecoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/machinecoin
SIGPATH=/some/root/path/machinecoin-detached-sigs

./bin/gbuild --url machinecoin=${BTCPATH},signature=${SIGPATH} ../machinecoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/machinecoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/machinecoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/machinecoin-osx-build.assert
```
...
[machinecoin-project/gitian.sigs.mac](https://github.com/machinecoin-project/gitian.sigs.mac/) repository, or if that's not possible create a pull
