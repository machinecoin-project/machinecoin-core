...
void CActiveDeterministicMasternodeManager::Init()
{
    ...
    if (!deterministicMNManager->AreDeterministicMNsActive()) return;
...
void CActiveDeterministicMasternodeManager::UpdatedBlockTip(const CBlockIndex* pindexNew, const CBlockIndex* pindexFork, bool fInitialDownload)
{
    ...
    if (!deterministicMNManager->AreDeterministicMNsActive(pindexNew->nHeight)) return;
...
void CActiveLegacyMasternodeManager::ManageState(CConnman& connman)
{
    ...
    LogPrint(MCLog::MN, "CActiveLegacyMasternodeManager::ManageState -- Start\n");
    if (!fMasternodeMode) {
        LogPrint(MCLog::MN, "CActiveLegacyMasternodeManager::ManageState -- Not a masternode, returning\n");
        return;
    }
    ...
    LogPrint(MCLog::MN, "CActiveLegacyMasternodeManager::ManageState -- status = %s, type = %s, pinger enabled = %d\n", GetStatus(), GetTypeString(), fPingerEnabled);
...
{
    if (deterministicMNManager->AreDeterministicMNsActive()) return false;

    if (!fPingerEnabled) {
        LogPrint(MCLog::MN, "CActiveLegacyMasternodeManager::SendMasternodePing -- %s: masternode ping service is disabled, skipping...\n", GetStateString());
        return false;
    }
...
void CActiveLegacyMasternodeManager::ManageStateInitial(CConnman& connman)
{
    ...
    LogPrint(MCLog::MN, "CActiveLegacyMasternodeManager::ManageStateInitial -- status = %s, type = %s, pinger enabled = %d\n", GetStatus(), GetTypeString(), fPingerEnabled);
    ...
    LogPrint(MCLog::MN, "CActiveLegacyMasternodeManager::ManageStateInitial -- End status = %s, type = %s, pinger enabled = %d\n", GetStatus(), GetTypeString(), fPingerEnabled);
}
...
void CActiveLegacyMasternodeManager::ManageStateRemote()
{
    ...
    LogPrint(MCLog::MN, "CActiveLegacyMasternodeManager::ManageStateRemote -- Start status = %s, type = %s, pinger enabled = %d, keyIDOperator = %s\n",
