...
UniValue masternode(const JSONRPCRequest& request)
{
    ...
    if (strCommand == "current" || strCommand == "winner")
    {
        ...
        obj.push_back(Pair("payee",         EncodeDestination(GetDestinationForKey(mnInfo.pubKeyCollateralAddress, OUTPUT_TYPE_LEGACY))));
    ...
    if (strCommand == "status")
    {
        ...
        if(mnodeman.Get(activeMasternode.outpoint, mn)) {
            mnObj.push_back(Pair("payee", EncodeDestination(GetDestinationForKey(mn.pubKeyCollateralAddress, OUTPUT_TYPE_LEGACY))));
        }
        ...
        for (auto& mnpair : mapMasternodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                streamFull << std::setw(18) <<
                               mn.GetStatus() << " " <<
                               mn.nProtocolVersion << " " <<
                               EncodeDestination(GetDestinationForKey(mn.pubKeyCollateralAddress, OUTPUT_TYPE_LEGACY)) << " " <<
                ...
                CTxDestination address = GetDestinationForKey(mn.pubKeyCollateralAddress, OUTPUT_TYPE_LEGACY);
    ...
    if (strCommand == "decode")
    {
        ...
        for (CMasternodeBroadcast& mnb : vecMnb) {
            ...
            if(mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("pubKeyCollateralAddress", EncodeDestination(GetDestinationForKey(mnb.pubKeyCollateralAddress, OUTPUT_TYPE_LEGACY))));
                resultObj.push_back(Pair("pubKeyMasternode", EncodeDestination(GetDestinationForKey(mnb.pubKeyMasternode, OUTPUT_TYPE_LEGACY))));
