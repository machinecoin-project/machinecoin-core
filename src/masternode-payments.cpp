...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, CTxOut& txoutMasternodeRet)
{
    ...
    if(!mnpayments.GetBlockPayee(nBlockHeight, payee)) {
        ...
        payee = GetScriptForDestination(WitnessV0KeyHash(mnInfo.pubKeyCollateralAddress.GetID()));
    }
...
bool CMasternodePayments::IsScheduled(CMasternode& mn, int nNotBlockHeight)
{
    ...
    CScript mnpayee;
    mnpayee = GetScriptForDestination(WitnessV0KeyHash(mn.pubKeyCollateralAddress.GetID()));
        ...
        if (payee.GetVoteCount() >= MNPAYMENTS_SIGNATURES_REQUIRED) {
            for (CTxOut txout : txNew->vout) {
                ...
                if (ExtractDestination(txout.scriptPubKey, dest)) {
                    auto witnessHash = boost::get<WitnessV0KeyHash>(&dest);
                    LogPrintf("Boost: %s\n", EncodeDestination(GetScriptForDestination(witnessHash)));
                }
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight, CConnman* connman)
{
    ...
    CScript payee = GetScriptForDestination(WitnessV0KeyHash(mnInfo.pubKeyCollateralAddress.GetID()));
