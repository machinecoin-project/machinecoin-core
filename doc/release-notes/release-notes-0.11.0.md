...
For example, add the following to `machinecoin.conf`:
...
related to the blockchain in the machinecoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Machinecoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Machinecoin Core will 
...
For an example script see [machinecoin-submittx](https://github.com/laanwj/machinecoin-submittx).
...
- #5334 `cf87536` libmachinecoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new machinecoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only MachinecoinAmountField
- #5753 `0f44672` Add machinecoin logo to about screen
...
- #5809 `46bfbe7` Add machinecoin-cli man page
...
- #5849 `41259ca` contrib/init/machinecoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Machinecoin-Qt.app to Machinecoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for machinecoin-tx)
