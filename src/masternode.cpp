...
{
    // NOTE not called when deterministic masternodes are activated
...
CMasternode::CollateralStatus CMasternode::CheckCollateral(const COutPoint& outpoint, const CKeyID& keyID, int& nHeightRet)
{
    ...
    if(keyID.IsNull() || coin.out.scriptPubKey != GetScriptForDestination(CScriptID(GetScriptForDestination(WitnessV0KeyHash(keyID))))) {
...
void CMasternode::UpdateLastPaid(const CBlockIndex *pindex, int nMaxBlocksToScanBack)
{
    ...
    if (deterministicMNManager->AreDeterministicMNsActive(pindex->nHeight)) {
        ...
        if (!dmn || dmn->pdmnState->nLastPaidHeight == -1) {
            LogPrint(MCLog::MN, "CMasternode::UpdateLastPaidBlock -- searching for block with payment to %s -- not found\n", outpoint.ToStringShort());
        } else {
    ...
    CScript mnpayee = GetScriptForDestination(CScriptID(GetScriptForDestination(WitnessV0KeyHash(keyIDCollateralAddress))));
...
bool CMasternodeBroadcast::SimpleCheck(int& nDos)
{
    ...
    CScript pubkeyScript = GetScriptForDestination(keyIDCollateralAddress);
    ...
    CScript pubkeyScript2 = GetScriptForDestination(legacyKeyIDOperator);
