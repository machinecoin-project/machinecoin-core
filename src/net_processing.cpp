...
static void PushNodeVersion(CNode *pnode, CConnman* connman, int64_t nTime)
{
    ...
    if (fLogIPs) {
        LogPrint(MCLog::NET, "send version message: version %d, blocks=%d, us=%s, them=%s, peer=%d\n", PROTOCOL_VERSION, nNodeStartingHeight, addrMe.ToString(), addrYou.ToString(), nodeid);
    } else {
        LogPrint(MCLog::NET, "send version message: version %d, blocks=%d, us=%s, peer=%d\n", PROTOCOL_VERSION, nNodeStartingHeight, addrMe.ToString(), nodeid);
    }
}
...
void PeerLogicValidation::FinalizeNode(NodeId nodeid, bool& fUpdateConnectionTime) {
    ...
    LogPrint(MCLog::NET, "Cleared nodestate for peer=%d\n", nodeid);
}
...
bool AddOrphanTx(const CTransactionRef& tx, NodeId peer) EXCLUSIVE_LOCKS_REQUIRED(g_cs_orphans)
{
    ...
    {
        LogPrint(MCLog::MEMPOOL, "ignoring large orphan tx (size: %u, hash: %s)\n", sz, hash.ToString());
        return false;
    }
    ...
    LogPrint(MCLog::MEMPOOL, "stored orphan tx %s (mapsz %u outsz %u)\n", hash.ToString(),
...
void EraseOrphansFor(NodeId peer)
{
    ...
    if (nErased > 0) LogPrint(MCLog::MEMPOOL, "Erased %d orphan tx from peer=%d\n", nErased, peer);
}
...
unsigned int LimitOrphanTxSize(unsigned int nMaxOrphans)
{
    ...
    if (nNextSweep <= nNow) {
        ...
        if (nErased > 0) LogPrint(MCLog::MEMPOOL, "Erased %d orphan tx due to expiration\n", nErased);
    }
...
void Misbehaving(NodeId pnode, int howmuch, const std::string& message) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    {
        LogPrint(MCLog::NET, "%s: %s peer=%d (%d -> %d) BAN THRESHOLD EXCEEDED%s\n", __func__, state->name, pnode, state->nMisbehavior-howmuch, state->nMisbehavior, message_prefixed);
        state->fShouldBan = true;
    } else
        LogPrint(MCLog::NET, "%s: %s peer=%d (%d -> %d)%s\n", __func__, state->name, pnode, state->nMisbehavior-howmuch, state->nMisbehavior, message_prefixed);
}
...
void PeerLogicValidation::BlockConnected(const std::shared_ptr<const CBlock>& pblock, const CBlockIndex* pindex, const std::vector<CTransactionRef>& vtxConflicted) {
    ...
    if (vOrphanErase.size()) {
        ...
        LogPrint(MCLog::MEMPOOL, "Erased %d orphan tx included or conflicted by block\n", nErased);
    }
...
void PeerLogicValidation::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock) {
    ...
    connman->ForEachNode([this, &pcmpctblock, pindex, &msgMaker, fWitnessEnabled, &hashBlock](CNode* pnode) {
        ...
        if (state.fPreferHeaderAndIDs && (!fWitnessEnabled || state.fWantsCmpctWitness) &&
            ...
            LogPrint(MCLog::NET, "%s sending header-and-ids %s to peer=%d\n", "PeerLogicValidation::NewPoWValidBlock",
...
void static ProcessGetBlockData(CNode* pfrom, const CChainParams& chainparams, const CInv& inv, CConnman* connman)
{
    ...
    if (need_activate_chain) {
        ...
        if (!ActivateBestChain(state, Params(), a_recent_block)) {
            LogPrint(MCLog::NET, "failed to activate chain (%s)\n", FormatStateMessage(state));
        }
    }
    ...
    if (pindex) {
        ...
        if (!send) {
            LogPrint(MCLog::NET, "%s: ignoring request from peer=%i for old block that isn't in the main chain\n", __func__, pfrom->GetId());
        }
    }
    ...
    {
        LogPrint(MCLog::NET, "historical block serving limit reached, disconnect peer=%d\n", pfrom->GetId());
    ...
    if (send && !pfrom->fWhitelisted && (
        ...
        LogPrint(MCLog::NET, "Ignore block request below NODE_NETWORK_LIMITED threshold from peer=%d\n", pfrom->GetId());
    ...
    const CNetMsgMaker msgMaker(pfrom->GetSendVersion());
    {
        ...
        while (it != pfrom->vRecvGetData.end() && (
            ...
            if (!push && inv.type == MSG_GOVERNANCE_OBJECT) {
                LogPrint(MCLog::NET, "ProcessGetData -- MSG_GOVERNANCE_OBJECT: inv = %s\n", inv.ToString());
                ...
                LogPrint(MCLog::NET, "ProcessGetData -- MSG_GOVERNANCE_OBJECT: topush = %d, inv = %s\n", topush, inv.ToString());
                if(topush) {
            ...
            if (!push && inv.type == MSG_GOVERNANCE_OBJECT_VOTE) {
                ...
                if(topush) {
                    LogPrint(MCLog::NET, "ProcessGetData -- pushing: inv = %s\n", inv.ToString());
...
bool static ProcessHeadersMessage(CNode *pfrom, CConnman *connman, const std::vector<CBlockHeader>& headers, const CChainParams& chainparams, bool punish_duplicate_invalid)
{
    ...
    const CBlockIndex *pindexLast = nullptr;
    {
        ...
        if (!LookupBlockIndex(headers[0].hashPrevBlock) && nCount < MAX_BLOCKS_TO_ANNOUNCE) {
            ...
            LogPrint(MCLog::NET, "received header %s: missing prev block %s, sending getheaders (%d) to end (peer=%d, nUnconnectingHeaders=%d)\n",
    ...
    if (!ProcessNewBlockHeaders(headers, state, chainparams, &pindexLast, &first_invalid_header)) {
        ...
        if (state.IsInvalid(nDoS)) {
            ...
            if (nDoS > 0) {
                ...
                LogPrint(MCLog::NET, "peer=%d: invalid header received\n", pfrom->GetId());
            }
...

    {
        ...
        if (nodestate->nUnconnectingHeaders > 0) {
            LogPrint(MCLog::NET, "peer=%d: resetting nUnconnectingHeaders (%d -> 0)\n", pfrom->GetId(), nodestate->nUnconnectingHeaders);
        }
        ...
        if (nCount == MAX_HEADERS_RESULTS) {
            ...
            // from there instead.
            LogPrint(MCLog::NET, "more getheaders (%d) to end to peer=%d (startheight:%d)\n", pindexLast->nHeight, pfrom->GetId(), pfrom->nStartingHeight);
        ...
        if (fCanDirectFetch && pindexLast->IsValid(BLOCK_VALID_TREE) && chainActive.Tip()->nChainWork <= pindexLast->nChainWork) {
            ...
            if (!chainActive.Contains(pindexWalk)) {
                LogPrint(MCLog::NET, "Large reorg, won't direct fetch to %s (%d)\n",
                ...
                for (const CBlockIndex *pindex : reverse_iterate(vToFetch)) {
                    ...
                    LogPrint(MCLog::NET, "Requesting block %s from  peer=%d\n",
                            pindex->GetBlockHash().ToString(), pfrom->GetId());
                }
                if (vGetData.size() > 1) {
                    LogPrint(MCLog::NET, "Downloading blocks toward %s (%d) via headers direct fetch\n",
        ...
        if (!pfrom->fDisconnect && IsOutboundDisconnectionCandidate(pfrom) && nodestate->pindexBestKnownBlock != nullptr) {
            ...
            if (g_outbound_peers_with_protect_from_disconnect < MAX_OUTBOUND_PEERS_TO_PROTECT_FROM_DISCONNECT && nodestate->pindexBestKnownBlock->nChainWork >= chainActive.Tip()->nChainWork && !nodestate->m_chain_sync.m_protect) {
                LogPrint(MCLog::NET, "Protecting outbound peer=%d from eviction\n", pfrom->GetId());
...
{
    LogPrint(MCLog::NET, "received: %s (%u bytes) peer=%d\n", SanitizeString(strCommand), vRecv.size(), pfrom->GetId());
    ...
    if (strCommand == NetMsgType::REJECT)
    {
        if (LogAcceptCategory(MCLog::NET)) {
            try {
                ...
                LogPrint(MCLog::NET, "Reject %s\n", SanitizeString(ss.str()));
            } catch (const std::ios_base::failure&) {
                // Avoid feedback loops by preventing reject messages from triggering a new reject message.
                LogPrint(MCLog::NET, "Unparseable reject message received\n");
            }
        }
        return true;
    }
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        {
            LogPrint(MCLog::NET, "peer=%d does not offer the expected services (%08x offered, %08x expected); disconnecting\n", pfrom->GetId(), nServices, GetDesirableServiceFlags(nServices));
            if (enable_bip61) {
        ...
        if (nVersion < MIN_PEER_PROTO_VERSION)
        {
            ...
            LogPrint(MCLog::NET, "peer=%d using obsolete version %i; disconnecting\n", pfrom->GetId(), nVersion);
            if (enable_bip61) {
        ...
        if (!pfrom->fInbound)
        {
            ...
            if (fListen && !IsInitialBlockDownload())
            {
                ...
                if (addr.IsRoutable())
                {
                    LogPrint(MCLog::NET, "ProcessMessages: advertising address %s\n", addr.ToString());
        ...
        LogPrint(MCLog::NET, "receive version message: %s: version %d, blocks=%d, us=%s, peer=%d%s\n",
        ...
        for (CInv &inv : vInv)
        {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    LogPrint(MCLog::NET, "getheaders (%d) %s to peer=%d\n", pindexBestHeader->nHeight, inv.hash.ToString(), pfrom->GetId());
                }
            }
            else
            {
                ...
                if (legacyMNObjs.count(inv.type) && deterministicMNManager->AreDeterministicMNsActive()) {
                    LogPrint(MCLog::NET, "ignoring (%s) inv of legacy type %d peer=%d\n", inv.hash.ToString(), inv.type, pfrom->GetId());
                    continue;
                }

                pfrom->AddInventoryKnown(inv);
                if (fBlocksOnly) {
                    LogPrint(MCLog::NET, "transaction (%s) inv sent in violation of protocol peer=%d\n", inv.hash.ToString(), pfrom->GetId());
    ...
    else if (strCommand == NetMsgType::GETDATA)
    {
        ...
        LogPrint(MCLog::NET, "received getdata (%u invsz) peer=%d\n", vInv.size(), pfrom->GetId());

        if (vInv.size() > 0) {
            LogPrint(MCLog::NET, "received getdata for: %s peer=%d\n", vInv[0].ToString(), pfrom->GetId());
        }
    ...
    else if (strCommand == NetMsgType::GETBLOCKS)
    {
        ...
        if (locator.vHave.size() > MAX_LOCATOR_SZ) {
            LogPrint(MCLog::NET, "getblocks locator size %lld > %d, disconnect peer=%d\n", locator.vHave.size(), MAX_LOCATOR_SZ, pfrom->GetId());
            pfrom->fDisconnect = true;
            return true;
        }
        ...
        int nLimit = 500;
        LogPrint(MCLog::NET, "getblocks %d to %s limit %d from peer=%d\n", (pindex ? pindex->nHeight : -1), hashStop.IsNull() ? "end" : hashStop.ToString(), nLimit, pfrom->GetId());
        for (; pindex; pindex = chainActive.Next(pindex))
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                LogPrint(MCLog::NET, "  getblocks stopping at %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                break;
            }
            ...
            {
                LogPrint(MCLog::NET, " getblocks stopping, pruned or too old block at %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                break;
            }
            ...
            if (--nLimit <= 0)
            {
                ...
                LogPrint(MCLog::NET, "  getblocks stopping at limit %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
    ...
    else if (strCommand == NetMsgType::GETBLOCKTXN)
    {
        ...
        if (!pindex || !(pindex->nStatus & BLOCK_HAVE_DATA)) {
            LogPrint(MCLog::NET, "Peer %d sent us a getblocktxn for a block we don't have\n", pfrom->GetId());
            return true;
        }
        ...
        if (pindex->nHeight < chainActive.Height() - MAX_BLOCKTXN_DEPTH) {
            ...
            LogPrint(MCLog::NET, "Peer %d sent us a getblocktxn for a block > %i deep\n", pfrom->GetId(), MAX_BLOCKTXN_DEPTH);
            CInv inv;
    ...
    else if (strCommand == NetMsgType::GETHEADERS)
    {
        ...
        if (locator.vHave.size() > MAX_LOCATOR_SZ) {
            LogPrint(MCLog::NET, "getheaders locator size %lld > %d, disconnect peer=%d\n", locator.vHave.size(), MAX_LOCATOR_SZ, pfrom->GetId());
            pfrom->fDisconnect = true;
            return true;
        }
        ...
        if (IsInitialBlockDownload() && !pfrom->fWhitelisted) {
            LogPrint(MCLog::NET, "Ignoring getheaders from peer=%d because node is in initial block download\n", pfrom->GetId());
            return true;
        }
        ...
        if (locator.IsNull())
        {
            ...
            if (!BlockRequestAllowed(pindex, chainparams.GetConsensus())) {
                LogPrint(MCLog::NET, "%s: ignoring request from peer=%i for old block header that isn't in the main chain\n", __func__, pfrom->GetId());
                return true;
            }
        }
        else
        ...
        LogPrint(MCLog::NET, "getheaders %d to %s from peer=%d\n", (pindex ? pindex->nHeight : -1), hashStop.IsNull() ? "end" : hashStop.ToString(), pfrom->GetId());
        for (; pindex; pindex = chainActive.Next(pindex))
        {
    ...
    else if (strCommand == NetMsgType::TX)
    {
        ...
        {
            LogPrint(MCLog::NET, "transaction sent in violation of protocol peer=%d\n", pfrom->GetId());
            return true;
        }
        ...
        if (!AlreadyHave(inv) &&
            ...
            LogPrint(MCLog::MEMPOOL, "AcceptToMemoryPool: peer=%d: accepted %s (poolsz %u txn, %u kB)\n",
                pfrom->GetId(),
                tx.GetHash().ToString(),
            ...
            while (!vWorkQueue.empty()) {
                     ...
                     ++mi)
                {
                    ...
                    if (AcceptToMemoryPool(mempool, stateDummy, porphanTx, &fMissingInputs2, &lRemovedTxn, false /* bypass_limits */, 0 /* nAbsurdFee */)) {
                        LogPrint(MCLog::MEMPOOL, "   accepted orphan tx %s\n", orphanHash.ToString());
                    ...
                    else if (!fMissingInputs2)
                    {
                        ...
                        if (stateDummy.IsInvalid(nDos) && nDos > 0)
                        {
                            ...
                            LogPrint(MCLog::MEMPOOL, "   invalid orphan tx %s\n", orphanHash.ToString());
                        }
                        // Has inputs but not accepted to mempool
                        // Probably non-standard or insufficient fee
                        LogPrint(MCLog::MEMPOOL, "   removed orphan tx %s\n", orphanHash.ToString());
                        vEraseQueue.push_back(orphanHash);
                        if (!orphanTx.HasWitness() && !stateDummy.CorruptionPossible()) {
                            // Do not use rejection cache for witness transactions or
                            // witness-stripped transactions, as they can have been malleated.
                            // See https://github.com/machinecoin-project/machinecoin-core/issues/8279 for details.
                            assert(recentRejects);
        ...
        else if (fMissingInputs)
        {
            ...
            if (!fRejectedParents) {
                ...
                if (nEvicted > 0) {
                    LogPrint(MCLog::MEMPOOL, "mapOrphan overflow, removed %u tx\n", nEvicted);
                }
            } else {
                LogPrint(MCLog::MEMPOOL, "not keeping orphan with rejected parents %s\n",tx.GetHash().ToString());
        ...
        if (state.IsInvalid(nDoS))
        {
            LogPrint(MCLog::MEMPOOLREJ, "%s from peer=%d was not accepted: %s\n", tx.GetHash().ToString(),
                pfrom->GetId(),
                FormatStateMessage(state));
        ...
        if (!ProcessNewBlockHeaders({cmpctblock.header}, state, chainparams, &pindex)) {
            ...
            if (state.IsInvalid(nDoS)) {
                if (nDoS > 0) {
                    ...
                    LogPrint(MCLog::NET, "Peer %d sent us invalid header via cmpctblock\n", pfrom->GetId());
                }
                return true;
            }
        }
            ...
            if ((!fAlreadyInFlight && nodestate->nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                ...
                if (!MarkBlockAsInFlight(pfrom->GetId(), pindex->GetBlockHash(), pindex, &queuedBlockIt)) {
                    if (!(*queuedBlockIt)->partialBlock)
                        ...
                        LogPrint(MCLog::NET, "Peer sent us compact block we were already syncing!\n");
                        return true;
                    }
                }
    ...
    else if (strCommand == NetMsgType::BLOCKTXN && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        bool fBlockRead = false;
        {
            ...
            if (it == mapBlocksInFlight.end() || !it->second.second->partialBlock ||
                ...
                LogPrint(MCLog::NET, "Peer %d sent us block transactions for block we weren't expecting\n", pfrom->GetId());
                return true;
            }
    ...
    else if (strCommand == NetMsgType::BLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        LogPrint(MCLog::NET, "received block %s peer=%d\n", pblock->GetHash().ToString(), pfrom->GetId());
    ...
    else if (strCommand == NetMsgType::GETADDR)
    {
        ...
        if (!pfrom->fInbound) {
            LogPrint(MCLog::NET, "Ignoring \"getaddr\" from outbound connection. peer=%d\n", pfrom->GetId());
            return true;
        }
        ...
        if (pfrom->fSentAddr) {
            LogPrint(MCLog::NET, "Ignoring repeated \"getaddr\". peer=%d\n", pfrom->GetId());
            return true;
        }
    ...
    else if (strCommand == NetMsgType::MEMPOOL)
    {
        ...
        {
            LogPrint(MCLog::NET, "mempool request with bloom filters disabled, disconnect peer=%d\n", pfrom->GetId());
            pfrom->fDisconnect = true;
            return true;
        }
        ...
        {
            LogPrint(MCLog::NET, "mempool request with bandwidth limit reached, disconnect peer=%d\n", pfrom->GetId());
            pfrom->fDisconnect = true;
            return true;
        }
    ...
    else if (strCommand == NetMsgType::PONG)
    {
        ...
        if (!(sProblem.empty())) {
            LogPrint(MCLog::NET, "pong peer=%d: %s, %x expected, %x received, %u bytes\n",
                pfrom->GetId(),
                sProblem,
    ...
    else if (strCommand == NetMsgType::FILTERCLEAR)
    {
        ...
        if (MoneyRange(newFeeFilter)) {
            ...
            LogPrint(MCLog::NET, "received: feefilter of %s from peer=%d\n", CFeeRate(newFeeFilter).ToString(), pfrom->GetId());
        }
    }
        ...
        if (BuildSimplifiedMNListDiff(cmd.baseBlockHash, cmd.blockHash, mnListDiff, strError)) {
            ...
            LogPrint(MCLog::NET, "getmnlistdiff failed for baseBlockHash=%s, blockHash=%s. error=%s\n", cmd.baseBlockHash.ToString(), cmd.blockHash.ToString(), strError);
            Misbehaving(pfrom->GetId(), 1);
        }
    }
        ...
        LOCK(cs_main);
        Misbehaving(pfrom->GetId(), 100);
        LogPrint(MCLog::NET, "received not-requested mnlistdiff. peer=%d\n", pfrom->GetId());
    }
    ...
    else
    {
        ...
        else
        {
            ...
            LogPrint(MCLog::NET, "Unknown command \"%s\" from peer=%d\n", SanitizeString(strCommand), pfrom->GetId());
        }
    }
...
bool PeerLogicValidation::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (memcmp(msg.hdr.pchMessageStart, chainparams.MessageStart(), CMessageHeader::MESSAGE_START_SIZE) != 0) {
        LogPrint(MCLog::NET, "PROCESSMESSAGE: INVALID MESSAGESTART %s peer=%d\n", SanitizeString(msg.hdr.GetCommand()), pfrom->GetId());
        pfrom->fDisconnect = true;
        return false;
    }
    ...
    {
        LogPrint(MCLog::NET, "PROCESSMESSAGE: ERRORS IN HEADER %s peer=%d\n", SanitizeString(hdr.GetCommand()), pfrom->GetId());
        return fMoreWork;
    }
    ...
    {
        LogPrint(MCLog::NET, "%s(%s, %u bytes): CHECKSUM ERROR expected %s was %s\n", __func__,
    ...
    catch (const std::ios_base::failure& e)
    {
        ...
        if (strstr(e.what(), "end of data"))
        {
            ...
            LogPrint(MCLog::NET, "%s(%s, %u bytes): Exception '%s' caught, normally caused by a message being shorter than its stated length\n", __func__, SanitizeString(strCommand), nMessageSize, e.what());
        }
        else if (strstr(e.what(), "size too large"))
        {
            // Allow exceptions from over-long size
            LogPrint(MCLog::NET, "%s(%s, %u bytes): Exception '%s' caught\n", __func__, SanitizeString(strCommand), nMessageSize, e.what());
        }
    ...
    if (!fRet) {
        LogPrint(MCLog::NET, "%s(%s, %u bytes) FAILED peer=%d\n", __func__, SanitizeString(strCommand), nMessageSize, pfrom->GetId());
    }
    ...
    if (!state.m_chain_sync.m_protect && IsOutboundDisconnectionCandidate(pto) && state.fSyncStarted) {
        ...
        if (state.pindexBestKnownBlock != nullptr && state.pindexBestKnownBlock->nChainWork >= chainActive.Tip()->nChainWork) {
            ...
            if (state.m_chain_sync.m_sent_getheaders) {
                ...
                LogPrint(MCLog::NET, "sending getheaders to outbound peer=%d to verify chain work (current best known block:%s, benchmark blockhash: %s)\n", pto->GetId(), state.pindexBestKnownBlock != nullptr ? state.pindexBestKnownBlock->GetBlockHash().ToString() : "<none>", state.m_chain_sync.m_work_header->GetBlockHash().ToString());
        ...
        if (worst_peer != -1) {
            bool disconnected = connman->ForNode(worst_peer, [&](CNode *pnode) {
                ...
                if (time_in_seconds - pnode->nTimeConnected > MINIMUM_CONNECT_TIME && state.nBlocksInFlight == 0) {
                    LogPrint(MCLog::NET, "disconnecting extra outbound peer=%d (last block announcement received at time %d)\n", pnode->GetId(), oldest_block_announcement);
                    pnode->fDisconnect = true;
                    return true;
                } else {
                    LogPrint(MCLog::NET, "keeping outbound peer=%d chosen for eviction (connect time: %d, blocks_in_flight: %d)\n", pnode->GetId(), pnode->nTimeConnected, state.nBlocksInFlight);
                    return false;
                }
            });
            if (disconnected) {
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 24 * 60 * 60) {
                ...
                LogPrint(MCLog::NET, "initial getheaders (%d) to peer=%d (startheight:%d)\n", pindexStart->nHeight, pto->GetId(), pto->nStartingHeight);
        ...
        //
        {
            ...
            if (!fRevertToInv && !vHeaders.empty()) {
                if (vHeaders.size() == 1 && state.fPreferHeaderAndIDs) {
                    ...
                    if (vHeaders.size() > 1) {
                        LogPrint(MCLog::NET, "%s: %u headers, range (%s, %s), to peer=%d\n", __func__,
                                vHeaders.size(),
                                vHeaders.front().GetHash().ToString(),
                                vHeaders.back().GetHash().ToString(), pto->GetId());
                    } else {
                        LogPrint(MCLog::NET, "%s: sending header %s to peer=%d\n", __func__,
                                vHeaders.front().GetHash().ToString(), pto->GetId());
                    }
            ...
            if (fRevertToInv) {
                ...
                if (!pto->vBlockHashesToAnnounce.empty()) {
                    ...
                    // Just log for now.
                    if (chainActive[pindex->nHeight] != pindex) {
                        LogPrint(MCLog::NET, "Announcing block %s not on main chain (tip=%s)\n",
                    ...
                    if (!PeerHasHeader(&state, pindex)) {
                        ...
                        LogPrint(MCLog::NET, "%s: sending inv peer=%d hash=%s\n", __func__,
        ...
        if (!pto->fClient && ((fFetch && !pto->m_limited_node) || !IsInitialBlockDownload()) && state.nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            ...
            for (const CBlockIndex *pindex : vToDownload) {
                ...
                LogPrint(MCLog::NET, "Requesting block %s (%d) peer=%d\n", pindex->GetBlockHash().ToString(),
                    pindex->nHeight, pto->GetId());
            }
            if (state.nBlocksInFlight == 0 && staller != -1) {
                if (State(staller)->nStallingSince == 0) {
                    State(staller)->nStallingSince = nNow;
                    LogPrint(MCLog::NET, "Stall started peer=%d\n", staller);
                }
            }
        }
        ...
        while (!pto->mapAskFor.empty() && (*pto->mapAskFor.begin()).first <= nNow)
        {
            ...
            if (!AlreadyHave(inv))
            {
                LogPrint(MCLog::NET, "Requesting %s peer=%d\n", inv.ToString(), pto->GetId());
