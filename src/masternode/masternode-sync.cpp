...
void CMasternodeSync::BumpAssetLastTime(const std::string& strFuncName)
{
    ...
    nTimeLastBumped = GetTime();
    LogPrint(MCLog::MNSYNC, "CMasternodeSync::BumpAssetLastTime -- %s\n", strFuncName);
}
...
void CMasternodeSync::ProcessTick(CConnman& connman)
{
    ...
    for (auto& pnode : vNodesCopy)
    {
        ...
        // NORMAL NETWORK MODE - TESTNET/MAINNET
        {
            ...
            if(nCurrentAsset == MASTERNODE_SYNC_WAITING) {
                if(pnode->nVersion >= 70024 && !pnode->fInbound && gArgs.GetBoolArg("-syncmempool", DEFAULT_SYNC_MEMPOOL) && !netfulfilledman.HasFulfilledRequest(pnode->addr, "mempool-sync")) {
            ...
            if(nCurrentAsset == MASTERNODE_SYNC_GOVERNANCE) {
                LogPrint(MCLog::GOBJECT, "CMasternodeSync::ProcessTick -- nTick %d nCurrentAsset %d nTimeLastBumped %lld GetTime() %lld diff %lld\n", nTick, nCurrentAsset, nTimeLastBumped, GetTime(), GetTime() - nTimeLastBumped);
...
{
    LogPrint(MCLog::MNSYNC, "CMasternodeSync::AcceptedBlockHeader -- pindexNew->nHeight: %d\n", pindexNew->nHeight);
...
{
    LogPrint(MCLog::MNSYNC, "CMasternodeSync::NotifyHeaderTip -- pindexNew->nHeight: %d fInitialDownload=%d\n", pindexNew->nHeight, fInitialDownload);
...
{
    LogPrint(MCLog::MNSYNC, "CMasternodeSync::UpdatedBlockTip -- pindexNew->nHeight: %d fInitialDownload=%d\n", pindexNew->nHeight, fInitialDownload);
    ...
    LogPrint(MCLog::MNSYNC, "CMasternodeSync::UpdatedBlockTip -- pindexNew->nHeight: %d pindexBestHeader->nHeight: %d fInitialDownload=%d fReachedBestHeader=%d\n",
