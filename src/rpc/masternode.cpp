// Copyright (c) 2014-2020 The Machinecoin developers
...
void masternode_list_help()
{
    throw std::runtime_error(
            ...
            "  owneraddress   - Print the masternode owner Machinecoin address\n"
            "  payee          - Print the masternode payout Machinecoin address (can be additionally filtered,\n"
            "                   partial match)\n"
            ...
            "  votingaddress  - Print the masternode voting Machinecoin address\n"
        );
}
...
UniValue GetNextMasternodeForPayment(int heightShift)
{
    ...
    if (deterministicMNManager->AreDeterministicMNsActive()) {
        ...
        if (!mnodeman.GetNextMasternodeInQueueForPayment(nHeight, true, nCount, mnInfo))
            return "unknown";
        payeeScript = GetScriptForDestination(CScriptID(GetScriptForDestination(WitnessV0KeyHash(mnInfo.keyIDCollateralAddress))));
    }

    CTxDestination payeeDest;
    std::string payeeAddr;
    if (ExtractDestination(payeeScript, payeeDest)) {
        payeeAddr = EncodeDestination(payeeDest);
    }
    ...
    obj.push_back(Pair("payee",         !payeeAddr.empty() ? payeeAddr : "UNKNOWN"));
...
UniValue masternode_genkey(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 1) {
        fCompressed = request.params[1].get_bool();
    }
...
UniValue masternode_status(const JSONRPCRequest& request)
{
    ...
    if (deterministicMNManager->AreDeterministicMNsActive()) {
        ...
        if (mnodeman.Get(activeMasternodeInfo.outpoint, mn)) {
            mnObj.push_back(Pair("payee", EncodeDestination(mn.keyIDCollateralAddress)));
        }
    ...
    if (strMode == "rank") {
        ...
        for (const auto& mnpair : mapMasternodes) {
            ...
            if (deterministicMNManager->AreDeterministicMNsActive()) {
                ...
                payeeScript = GetScriptForDestination(CScriptID(GetScriptForDestination(WitnessV0KeyHash(mn.keyIDCollateralAddress))));
                collateralAddressStr = EncodeDestination(mn.keyIDCollateralAddress);
            }
            ...
            if (strMode == "activeseconds") {
                ...
                objMN.push_back(Pair("owneraddress", EncodeDestination(mn.keyIDOwner)));
                objMN.push_back(Pair("votingaddress", EncodeDestination(mn.keyIDVoting)));
                ...
                if (strFilter !="" && strOutpoint.find(strFilter) == std::string::npos) continue;
                obj.push_back(Pair(strOutpoint, EncodeDestination(mn.keyIDOwner)));
                ...
                if (strFilter !="" && strOutpoint.find(strFilter) == std::string::npos) continue;
                obj.push_back(Pair(strOutpoint, EncodeDestination(mn.keyIDVoting)));
            }
        }
    }
    return obj;
    ...
    if (strCommand == "decode") {
        ...
        for (const auto& mnb : vecMnb) {
            ...
            if (mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("keyIDCollateralAddress", EncodeDestination(mnb.keyIDCollateralAddress)));
                resultObj.push_back(Pair("keyIDMasternode", EncodeDestination(mnb.legacyKeyIDOperator)));
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    { "machinecoin",               "masternode",             &masternode,             {} },
    { "machinecoin",               "masternodelist",         &masternodelist,         {} },
    { "machinecoin",               "masternodebroadcast",    &masternodebroadcast,    {} },
    { "machinecoin",               "sentinelping",           &sentinelping,           {} },
};
