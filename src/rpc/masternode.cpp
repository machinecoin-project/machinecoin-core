    ...
    if (strCommand == "decode")
    {
        ...
        for (CMasternodeBroadcast& mnb : vecMnb) {
            ...
            if(mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("pubKeyCollateralAddress", EncodeDestination(CScript(GetScriptForDestination(WitnessV0KeyHash(mnb.pubKeyCollateralAddress.GetID()))))));
                resultObj.push_back(Pair("pubKeyMasternode", EncodeDestination(CScript(GetScriptForDestination(WitnessV0KeyHash(mnb.pubKeyMasternode.GetID()))))));
