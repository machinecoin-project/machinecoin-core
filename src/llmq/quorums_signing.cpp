...
void CRecoveredSigsDb::CleanupOldRecoveredSigs(int64_t maxAge)
{
    ...
    LogPrint(MCLog::LLMQ, "CRecoveredSigsDb::%d -- deleted %d entries\n", __func__, toDelete.size());
}
...
void CSigningManager::ProcessMessageRecoveredSig(CNode* pfrom, const CRecoveredSig& recoveredSig, CConnman& connman)
{
    ...
    LogPrint(MCLog::LLMQ, "CSigningManager::%s -- signHash=%s, id=%s, msgHash=%s, node=%d\n", __func__,
...
bool CSigningManager::PreVerifyRecoveredSig(NodeId nodeId, const CRecoveredSig& recoveredSig, bool& retBan)
{
    ...
    if (!quorum) {
        LogPrint(MCLog::LLMQ, "CSigningManager::%s -- quorum %s not found, node=%d\n", __func__,
    ...
    for (auto& p : retSigShares) {
        ...
        for (auto it = v.begin(); it != v.end();) {
            ...
            if (!retQuorums.count(quorumKey)) {
                ...
                if (!CLLMQUtils::IsQuorumActive(llmqType, quorum->qc.quorumHash)) {
                    LogPrint(MCLog::LLMQ, "CSigningManager::%s -- quorum %s not active anymore, node=%d\n", __func__,
                              recSig.quorumHash.ToString(), nodeId);
                    it = v.erase(it);
                    continue;
                }
...
bool CSigningManager::ProcessPendingRecoveredSigs(CConnman& connman)
{
    ...
    LogPrint(MCLog::LLMQ, "CSigningManager::%s -- verified recovered sig(s). count=%d, vt=%d, nodes=%d\n", __func__, verifyCount, verifyTimer.count(), recSigsByNode.size());
...
void CSigningManager::ProcessRecoveredSig(NodeId nodeId, const CRecoveredSig& recoveredSig, const CQuorumCPtr& quorum, CConnman& connman)
{
    ...
    std::vector<CRecoveredSigsListener*> listeners;
    {
        ...
        LogPrint(MCLog::LLMQ, "CSigningManager::%s -- valid recSig. signHash=%s, id=%s, msgHash=%s, node=%d\n", __func__,
...
bool CSigningManager::AsyncSignIfMember(Consensus::LLMQType llmqType, const uint256& id, const uint256& msgHash, bool allowReSign)
{
...

    {
        ...
        if (hasVoted) {
            ...
            if (msgHash != prevMsgHash) {
                ...
                LogPrint(MCLog::LLMQ, "CSigningManager::%s -- already voted for id=%s and msgHash=%s. Resigning!\n", __func__,
                         id.ToString(), prevMsgHash.ToString());
            } else {
                LogPrint(MCLog::LLMQ, "CSigningManager::%s -- already voted for id=%s and msgHash=%s. Not voting again.\n", __func__,
                          id.ToString(), prevMsgHash.ToString());
                return false;
            }
        }
    ...
    if (!quorum) {
        LogPrint(MCLog::LLMQ, "CSigningManager::%s -- failed to select quorum. id=%s, msgHash=%s\n", __func__, id.ToString(), msgHash.ToString());
        return false;
    }
