...
void static ProcessGetData(CNode* pfrom, const Consensus::Params& consensusParams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    const CNetMsgMaker msgMaker(pfrom->GetSendVersion());
    {
        ...
        while (it != pfrom->vRecvGetData.end() && (it->type == MSG_TX || it->type == MSG_WITNESS_TX || it->type == MSG_MASTERNODE_PAYMENT_VOTE || it->type == MSG_MASTERNODE_PAYMENT_BLOCK || it->type == MSG_MASTERNODE_QUORUM || it->type == MSG_MASTERNODE_ANNOUNCE || it->type == MSG_MASTERNODE_PING || it->type == MSG_GOVERNANCE_OBJECT || it->type == MSG_GOVERNANCE_OBJECT_VOTE || it->type == MSG_MASTERNODE_VERIFY)) {
            if (interruptMsgProc)
                return;
            ...
            if (!push && (inv.type == MSG_TX || inv.type == MSG_WITNESS_TX)) {
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::GETBLOCKTXN)
    {
        ...
        if (it == mapBlockIndex.end() || !(it->second->nStatus & BLOCK_HAVE_DATA)) {
            LogPrint(MCLog::NET, "Peer %d sent us a getblocktxn for a block we don't have", pfrom->GetId());
            return true;
        }
        ...
        if (it->second->nHeight < chainActive.Height() - MAX_BLOCKTXN_DEPTH) {
            ...
            LogPrint(MCLog::NET, "Peer %d sent us a getblocktxn for a block > %i deep", pfrom->GetId(), MAX_BLOCKTXN_DEPTH);
            CInv inv;
        ...
        std::vector<CInv> vInv;
        {
            ...
            if (fSendTrickle && pto->fSendMempool) {
                ...
                for (const auto& txinfo : vtxinfo) {
                    ...
                    LogPrint(MCLog::NET, "SendMessages -- queued inv: %s  index=%d peer=%d\n", inv.ToString(), vInv.size(), pto->GetId());
                    vInv.push_back(inv);
                    if (vInv.size() == MAX_INV_SZ) {
                        LogPrint(MCLog::NET, "SendMessages -- pushing inv's: count=%d peer=%d\n", vInv.size(), pto->GetId());
                        connman->PushMessage(pto, msgMaker.Make(NetMsgType::INV, vInv));
                        vInv.clear();
                    }
                }
