...
The RPM spec file provided here is for Machinecoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://machinecoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Machinecoin git repository](https://github.com/machinecoin-project/machinecoin-core) but are not part of
...
Currently only `Source10` is used, for the example `machinecoin.conf` file.
...
in the machinecoin git repository. At present only an SVG version of the Machinecoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the machinecoin
...
needed when 0.12.1 is released, a proper fix is already in the Machinecoin git
master branch.
...
idea to build Machinecoin Core against a static version of that library compiled
...
The `machinecoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `machinecoin-server` package. At this point, most RPM based
...
The `machinecoin-server` package should have SELinux support. How to properly do
...
Machinecoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/machinecoin/). He has been
packaging Machinecoin for Fedora at least since 2012.
...
4. I renamed the `machinecoin` package that contains the Qt GUI to `machinecoin-core` as
that appears to be how the general population refers to it, in contrast to
`machinecoin-xt` or `machinecoin-classic`. I wanted to make sure the general population
