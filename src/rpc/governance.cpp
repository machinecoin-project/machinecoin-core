...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 MACHINECOIN irreversibly.
    ...
    LogPrint(MCLog::GOBJECT, "gobject_prepare -- GetDataAsPlainString = %s, hash = %s, txid = %s\n",
...
UniValue gobject_submit(const JSONRPCRequest& request)
{
    ...
    LogPrint(MCLog::GOBJECT, "gobject_submit -- pubKeyOperator = %s, outpoint = %s, params.size() = %lld, fMnFound = %d\n",
    ...
    LogPrint(MCLog::GOBJECT, "gobject_submit -- GetDataAsPlainString = %s, hash = %s, txid = %s\n",
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in MACHINECOIN.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("MACHINECOIN.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in MACHINECOIN.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)        argNames
  //  --------------------- ------------------------  ----------------------- ----------
    ...
    { "machinecoin",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "machinecoin",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "machinecoin",               "gobject",                &gobject,                {} },
    { "machinecoin",               "voteraw",                &voteraw,                {} },
