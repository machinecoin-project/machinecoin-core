...
Also test that nodes that send unsupported service bits to machinecoind are disconnected
...
class CNodeNoVersionBan(CLazyNode):
    ...
    # NOTE: implementation-specific check here. Remove if machinecoind ban behavior changes
    def on_open(self):
        super().on_open()
...
class P2PLeakTest(MachinecoinTestFramework):
