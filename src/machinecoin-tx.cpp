...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s machinecoin-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  machinecoin-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded machinecoin transaction") + "\n" +
              "  machinecoin-tx [options] -create [commands]   " + _("Create hex-encoded machinecoin transaction") + "\n" +
              "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CMachinecoinAddress addr(strAddr);
    if (!addr.IsValid())
...
static void MutateTxAddOutPubKey(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CMachinecoinAddress addr(scriptPubKey);
    ...
    if (bScriptHash) {
        ...
        CMachinecoinAddress redeemScriptAddr(scriptPubKey);
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded machinecoin transaction
