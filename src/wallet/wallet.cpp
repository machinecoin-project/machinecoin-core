...
void CWallet::AvailableMNCoins(std::vector<COutput> &vCoins, bool fOnlySafe, const CCoinControl *coinControl, const CAmount &nMinimumAmount, const CAmount &nMaximumAmount, const CAmount &nMinimumSumAmount, const uint64_t nMaximumCount, const int nMinDepth, const int nMaxDepth) const
{
    vCoins.clear();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/machinecoin/machinecoin/pull/10340, but that change has been
    // postponed until after https://github.com/machinecoin/machinecoin/pull/10244 to
                                 ...
                                 std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet) const
{
        ...
        ((nBest != nTargetValue && nBest < nTargetValue + MIN_CHANGE) || coinLowestLarger->txout.nValue <= nBest))
    {
        ...
        if (LogAcceptCategory(MCLog::SELECTCOINS)) {
            LogPrint(MCLog::SELECTCOINS, "SelectCoins() best subset: ");
            for (unsigned int i = 0; i < vValue.size(); i++) {
                if (vfBest[i]) {
                    LogPrint(MCLog::SELECTCOINS, "%s ", FormatMoney(vValue[i].txout.nValue));
                }
            }
            LogPrint(MCLog::SELECTCOINS, "total %s\n", FormatMoney(nBest));
        }
    }
 ...
 * https://machinecointalk.org/?topic=54527, or
 * https://github.com/machinecoin/machinecoin/pull/1393.
 */
