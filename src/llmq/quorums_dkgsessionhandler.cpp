...
void CDKGPendingMessages::PushPendingMessage(NodeId from, CDataStream& vRecv)
{
    ...
    if (!seenMessages.emplace(hash).second) {
        LogPrint(MCLog::LLMQ_DKG, "CDKGPendingMessages::%s -- already seen %s, peer=%d\n", __func__, hash.ToString(), from);
        return;
    }
...
void CDKGSessionHandler::HandleDKGRound()
{
    ...
    if (curSession->AreWeMember() || gArgs.GetBoolArg("-watchquorums", DEFAULT_WATCH_QUORUMS)) {
        ...
        if (!connections.empty()) {
            if (LogAcceptCategory(MCLog::LLMQ_DKG)) {
...
void CDKGSessionHandler::PhaseHandlerThread()
{
    while (!stopRequested && !ShutdownRequested()) {
        try {
            ...
            LogPrint(MCLog::LLMQ_DKG, "CDKGSessionHandler::%s -- aborted current DKG session for llmq=%s\n", __func__, params.name);
        }
    }
}
