...
- The `estimatesmartfee` RPC interface has been changed (See [PR 10707](https://github.com/machinecoin/machinecoin/pull/10707)):
    ...
    - If Machinecoin Core has not been running for long enough and has not seen enough blocks or transactions to produce an accurate fee estimation, an error will be returned (previously a value of -1 was used to indicate an error, which could be confused for a feerate).
...
**In Machinecoin-Qt, only the first wallet will be displayed and accessible for creating and signing transactions.** GUI selectable multiple wallets will be supported in a future version. However, even in 0.15 other loaded wallets will remain synchronized to the node's current tip in the background. This can be useful if running a pruned node, since loading a wallet where the most recent sync is beyond the pruned height results in having to download and revalidate the whole blockchain. Continuing to synchronize all wallets in the background avoids this problem.

Machinecoin Core 0.15.0 contains the following changes to the RPC interface and `machinecoin-cli` for multi-wallet:

* When running Machinecoin Core with a single wallet, there are **no** changes to the RPC interface or `machinecoin-cli`. All RPC calls and `machinecoin-cli` commands continue to work as before.
* When running Machinecoin Core with multi-wallet, all *node-level* RPC methods continue to work as before. HTTP RPC requests should be send to the normal `<RPC IP address>:<RPC port>` endpoint, and `machinecoin-cli` commands should be run as before. A *node-level* RPC method is any method which does not require access to the wallet.
* When running Machinecoin Core with multi-wallet, *wallet-level* RPC methods must specify the wallet for which they're intended in every request. HTTP RPC requests should be send to the `<RPC IP address>:<RPC port>/wallet/<wallet name>` endpoint, for example `127.0.0.1:8332/wallet/wallet1.dat`. `machinecoin-cli` commands should be run with a `-rpcwallet` option, for example `machinecoin-cli -rpcwallet=wallet1.dat getbalance`.
...
Machinecoin Core has supported creating opt-in replace-by-fee (RBF) transactions
...
- When using Machinecoin Core in multi-wallet mode, RPC requests for wallet methods must specify
...
- Error codes have been updated to be more accurate for the following error cases (See [PR 9853](https://github.com/machinecoin/machinecoin/pull/9853)):
  ...
  - `removeprunedfunds` now returns RPC_WALLET_ERROR if `machinecoind` is unable to remove
  the transaction. Previously returned RPC_INTERNAL_ERROR.
  - `removeprunedfunds` now returns RPC_INVALID_PARAMETER if the transaction does not
  exist in the wallet. Previously returned RPC_INTERNAL_ERROR.
  - `fundrawtransaction` now returns RPC_INVALID_ADDRESS_OR_KEY if an invalid change
  address is provided. Previously returned RPC_INVALID_PARAMETER.
  - `fundrawtransaction` now returns RPC_WALLET_ERROR if `machinecoind` is unable to create
...
- #10475 `39039b1` [RPC] getmempoolinfo mempoolminfee is a MAC/KB feerate (instagibbs)
...
- #9672 `46311e7` Opt-into-RBF for RPC & machinecoin-tx (luke-jr)
...
- #10400 `1680ee0` [RPC] Add an uptime command that displays the amount of time (in seconds) machinecoind has been running (rvelhote)
- #10683 `d81bec7` rpc: Move the `generate` RPC call to rpcwallet (laanwj)
- #10710 `30bc0f6` REST/RPC example update (Mirobit)
- #10747 `9edda0c` [rpc] fix verbose argument for getblock in machinecoin-cli (jnewbery)
...
- #9805 `5b583ef` Add seed.mac.petertodd.org to mainnet DNS seeds (petertodd)
...
- #10228 `27faa6c` build: regenerate machinecoin-config.h as necessary (theuni)
...
- #10017 `02d64bd` combine_logs.py - aggregates log files from multiple machinecoinds during functional tests (jnewbery)
...
- #10142 `ed09dd3` Run machinecoin_test-qt under minimal QPA platform (ryanofsky)
...
- #10359 `329fc1d` [tests] functional tests should call MachinecoinTestFramework start/stop node methods (jnewbery)
...
- #10636 `4bc853b` [qa] util: Check return code after closing machinecoind proc (MarcoFalke)
...
- #10556 `65cc7aa` Move stop/start functions from utils.py into MachinecoinTestFramework (jnewbery)
...
- #10090 `8e4f7e7` Update machinecoin.conf with example for pruning (coinables)
...
- #10447 `f259263` Make machinecoind invalid argument error message specific (laanwj)
- #10495 `6a38b79` contrib: Update location of seeds.txt (laanwj)
- #10469 `b6b150b` Fixing typo in rpcdump.cpp help message (keystrike)
- #10451 `27b9931` contrib/init/machinecoind.openrcconf: Don't disable wallet by default (luke-jr)
...
- #9999 `a328904` [LevelDB] Plug leveldb logs to machinecoin logs (NicolasDorier)
...
- #10958 `659c096` Update to latest Machinecoin patches for LevelDB (sipa)
