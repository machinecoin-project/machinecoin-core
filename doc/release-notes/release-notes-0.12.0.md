...
pre-0.12 versions of Machinecoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Machinecoin Core version 0.11 or
earlier.
...
[libsecp256k1](https://github.com/machinecoin-project/secp256k1) instead of OpenSSL.
...
(https://github.com/machinecoin-project/bips/blob/master/bip-0130.mediawiki)
...
Machinecoin Core 0.12 will have a strict maximum size on the mempool. The
...
Machinecoin Core 0.12 also introduces new default policy limits on the length and
...
Machinecoin Core 0.12 nodes. Machinecoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/machinecoin-project/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Machinecoin Core 0.12 does not yet have support for
...
111](https://github.com/machinecoin-project/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/machinecoin-project/bips/blob/master/bip-0065.mediawiki)
...
Then, to tunnel a SSL connection on 60332 to a RPC server bound on localhost on port 50332 do:

    stunnel -d 60332 -r 127.0.0.1:50332 -p stunnel.pem -P ''
...
- #6214 `87406aa` [OSX] revert renaming of Machinecoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new machinecoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include machinecoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix machinecoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of machinecoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override MACHINECOIND and MACHINECOINCLI if they're set (Wladimir J. van der Laan)
...
- #6430 `b01981e` doc: add documentation for shared library libmachinecoinconsensus (Braydon Fuller)
