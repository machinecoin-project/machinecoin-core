...
bool AppInitMain()
{
...
#endif
    if (gArgs.GetBoolArg("-shrinkdebugfile", logCategories == MCLog::NONE)) {
    ...
    if(fMasterNode) {
        ...
        if(!strMasterNodePrivKey.empty()) {
            ...
            LogPrintf("  pubKeyMasternode: %s\n", EncodeDestination(GetScriptForDestination(WitnessV0KeyHash(pubKeyCollateralAddress.GetID()))));
        } else {
    ...
    connOptions.nMaxMasternodeOutbound = MAX_OUTBOUND_MASTERNODE_CONNECTIONS;
