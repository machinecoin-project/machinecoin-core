...
case $LEVEL in
  0)
    echo "Error: no Machinecoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Machinecoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Machinecoin 0.7 datadir."
    ;;
  3)
    echo "Detected Machinecoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Machinecoin 0.8 datadir."
    ;;
esac
