...
bool IsBlockValueValid(const CBlock& block, int nBlockHeight, CAmount blockReward, std::string &strErrorRet)
{
    ...
    if(!masternodeSync.IsSynced()) {
        ...
        if(CSuperblock::IsValidBlockHeight(nBlockHeight)) {
            LogPrint(MCLog::MN, "IsBlockPayeeValid -- WARNING: Client not synced, checking superblock max bounds only\n");
...
bool IsBlockPayeeValid(const CTransactionRef& txNew, int nBlockHeight, CAmount blockReward)
{
    if(!masternodeSync.IsSynced()) {
        ...
        LogPrint(MCLog::MN, "IsBlockPayeeValid -- WARNING: Client not synced, skipping block payee checks\n");
        return true;
    }
...
void CMasternodePayments::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman* connman)
{
    ...
    if (strCommand == NetMsgType::MASTERNODEPAYMENTSYNC) { //Masternode Payments Request Sync
        ...
        Sync(pfrom, connman);
        LogPrint(MCLog::MN, "MASTERNODEPAYMENTSYNC -- Sent Masternode payment votes to peer %d\n", pfrom->GetId());
...

        {
            ...
            if(mapMasternodePaymentVotes.count(nHash)) {
                LogPrint(MCLog::MN, "MASTERNODEPAYMENTVOTE -- hash=%s, nHeight=%d seen\n", nHash.ToString(), nCachedBlockHeight);
                return;
            }
        ...
        if(!CanVote(vote.vinMasternode.prevout, vote.nBlockHeight)) {
            LogPrint(MCLog::MN, "MASTERNODEPAYMENTVOTE -- masternode already voted, masternode=%s\n", vote.vinMasternode.prevout.ToStringShort());
            return;
        }
        ...
        if(!mnodeman.GetMasternodeInfo(vote.vinMasternode.prevout, mnInfo)) {
            ...
            LogPrint(MCLog::MN, "MASTERNODEPAYMENTVOTE -- masternode is missing %s\n", vote.vinMasternode.prevout.ToStringShort());
        ...
        LogPrint(MCLog::MN, "MASTERNODEPAYMENTVOTE -- vote: address=%s, nBlockHeight=%d, nHeight=%d, prevout=%s, hash=%s new\n",
                    EncodeDestination(address1), vote.nBlockHeight, nCachedBlockHeight, vote.vinMasternode.prevout.ToStringShort(), nHash.ToString());
...
bool CMasternodePaymentVote::Sign()
{
    ...
    if(!CMessageSigner::SignMessage(strMessage, vchSig, activeMasternode.keyMasternode)) {
        LogPrint(MCLog::MN, "CMasternodePaymentVote::Sign -- SignMessage() failed\n");
        return false;
    }
...
bool CMasternodePayments::IsScheduled(CMasternode& mn, int nNotBlockHeight)
{
    ...
    CScript mnpayee;
    mnpayee = GetScriptForDestination(CScriptID(GetScriptForDestination(WitnessV0KeyHash(mn.pubKeyCollateralAddress.GetID()))));
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight, CConnman* connman)
{
    ...
    LogPrint(MCLog::MN, "CMasternodePayments::ProcessBlock -- Masternode found by GetNextMasternodeInQueueForPayment(): %s\n", mnInfo.vin.prevout.ToStringShort());
    ...
    if (voteNew.Sign()) {
        LogPrint(MCLog::MN, "CMasternodePayments::ProcessBlock -- AddPaymentVote()\n");
...
void CMasternodePayments::CheckPreviousBlockVotes(int nPrevBlockHeight)
{
    ...
    for (int i = 0; i < MNPAYMENTS_SIGNATURES_TOTAL && i < (int)mns.size(); i++) {
        ...
        debugStr += strprintf("CMasternodePayments::CheckPreviousBlockVotes --   %s - voted for %s\n",
                              mn.second.vin.prevout.ToStringShort(), EncodeDestination(address1));
    }
...
void CMasternodePayments::RequestLowDataPaymentBlocks(CNode* pnode, CConnman* connman)
{
    ...
    while(nCachedBlockHeight - pindex->nHeight < nLimit) {
        if(!mapMasternodeBlocks.count(pindex->nHeight)) {
            ...
            if(vToFetch.size() == MAX_INV_SZ) {
                LogPrint(MCLog::MN, "CMasternodePayments::SyncLowDataPaymentBlocks -- asking peer %d for %d blocks\n", pnode->GetId(), MAX_INV_SZ);
    ...
    while(it != mapMasternodeBlocks.end()) {
        ...
        // We should not violate GETDATA rules
        if(vToFetch.size() == MAX_INV_SZ) {
            LogPrint(MCLog::MN, "CMasternodePayments::SyncLowDataPaymentBlocks -- asking peer %d for %d payment blocks\n", pnode->GetId(), MAX_INV_SZ);
            connman->PushMessage(pnode, msgMaker.Make(NetMsgType::GETDATA, vToFetch));
            // connman->PushMessage(pnode, NetMsgType::GETDATA, vToFetch);
            // Start filling new batch
            vToFetch.clear();
