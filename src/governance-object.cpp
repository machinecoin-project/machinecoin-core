...
bool CGovernanceObject::Sign(CKey& keyMasternode, CPubKey& pubKeyMasternode)
{
    ...
    if(!CMessageSigner::SignMessage(strMessage, vchSig, keyMasternode)) {
        LogPrint(MCLog::GOV, "CGovernanceObject::Sign -- SignMessage() failed\n");
        return false;
    }

    if(!CMessageSigner::VerifyMessage(pubKeyMasternode, vchSig, strMessage, strError)) {
        LogPrint(MCLog::GOV, "CGovernanceObject::Sign -- VerifyMessage() failed, error: %s\n", strError);
        return false;
    }

    LogPrint(MCLog::GOV, "CGovernanceObject::Sign -- pubkey id = %s, vin = %s\n",
...
bool CGovernanceObject::CheckSignature(CPubKey& pubKeyMasternode)
{
    ...
    LOCK(cs);
    if(!CMessageSigner::VerifyMessage(pubKeyMasternode, vchSig, strMessage, strError)) {
        LogPrint(MCLog::GOV, "CGovernance::CheckSignature -- VerifyMessage() failed, error: %s\n", strError);
        return false;
    }
...
bool CGovernanceObject::IsCollateralValid(std::string& strError, bool& fMissingConfirmations)
{
    ...
    for (const CTxOut o : txCollateral->vout) {
        if(!o.scriptPubKey.IsPayToScriptHash() && !o.scriptPubKey.IsUnspendable()) {
