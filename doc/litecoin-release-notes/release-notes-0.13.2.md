Machinecoin Core version 0.13.2 is now available from:
  ...
  <https://download.machinecoin.org/machinecoin-0.13.2.1/>
...
ECDSA signatures inside Machinecoin transactions now use validation using
[libsecp256k1](https://github.com/machinecoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/machinecoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Machinecoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Machinecoin Core was to raise the default minimum
relay fee.

Machinecoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Machinecoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running machinecoind without having to do any manual configuration.
...
Machinecoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Machinecoin Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Machinecoin Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Machinecoin Core 0.13, and it is at this time undecided whether the
...
Machinecoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Machinecoin Core automatically creates a hidden service to listen on, without
manual configuration. Machinecoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Machinecoin Core is listening, and
...
Machinecoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Machinecoin
Core to use floating fees. By default, Machinecoin Core will use floating
fees.
...
`machinecoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of Machinecoin Core v0.11.0](https://github.com/machinecoin-project/machinecoin-core/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/machinecoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/machinecoin/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /machinecoinrpc>
        ...
        # optional bypass machinecoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< machinecoinrpc:<password>
    </Location>
...
Restarting machinecoind will no longer clear out the list of banned peers; instead
...
machinecoin-cli: arguments privacy
------------------------------
...
- `machinecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `machinecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Machinecoin scripting system that in combination with
...
Machinecoin transactions currently may specify a locktime indicating when
...
The two phases are distinct in the Machinecoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Machinecoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Machinecoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Machinecoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of machinecoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of machinecoins being spent is exactly
...
- **Increased security for multisig:** Machinecoin addresses (both P2PKH addresses
  ...
  believed to be provided by Machinecoin's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  Machinecoin paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of Machinecoin's current ledger (called the UTXO set),
  ...
  note that Machinecoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Machinecoin users to individually opt-in to almost any change in the Machinecoin
  Script language when those users receive new transactions.  Features
  currently being researched by Machinecoin and Machinecoin Core contributors that may
  ...
  Other Machinecoin community members are studying several other improvements
...
segwit's start date of 1 January 2017 miners can update the Machinecoin
client to Machinecoin Core 0.13.2 to signal for segwit support. When a
...
Since Machinecoin Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/machinecoin/bips/blob/master/bip-0147.mediawiki
...
- [The Machinecoin Core Developers](/doc/release-notes)
- Charles Lee
- Adrian Gallagher
- shaolinfry
- Xinxi Wang
