    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwalletMain && pwalletMain->GetCScript(scriptID, subscript)) {
            ...
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                a.push_back(CMachinecoinAddress(addr).ToString());
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nReturn information about the given machinecoin address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The machinecoin address to validate\n"
            "\nResult:\n"
            "{\n"
            ...
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("validateaddress", "\"MSnWX92pkmSby4mzXtTeBAwzBaMFWzVS2q\"")
            + HelpExampleRpc("validateaddress", "\"MSnWX92pkmSby4mzXtTeBAwzBaMFWzVS2q\"")
        );
...
#ifdef ENABLE_WALLET
        // Case 1: Machinecoin address and we have full public key:
        CMachinecoinAddress address(ks);
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are machinecoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) machinecoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
            ...
            + HelpExampleCli("createmultisig", "2 \"[\\\"MSnWX92pkmSby4mzXtTeBAwzBaMFWzVS2q\\\",\\\"MHjxMWzfSsjrLYrVRsBsbgDtZZyoxNi6b3\\\"]\"") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("createmultisig", "2, \"[\\\"MSnWX92pkmSby4mzXtTeBAwzBaMFWzVS2q\\\",\\\"MHjxMWzfSsjrLYrVRsBsbgDtZZyoxNi6b3\\\"]\"")
        ;
    ...
    CMachinecoinAddress address(innerID);
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The machinecoin address to use for the signature.\n"
            ...
            "\nAs json rpc\n"
            + HelpExampleRpc("verifymessage", "\"MSnWX92pkmSby4mzXtTeBAwzBaMFWzVS2q\", \"signature\", \"my message\"")
        );
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw runtime_error(
            ...
            "machinecoin-cli and the GUI. There is no server-side difference."
        );
